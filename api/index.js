...
app.get('/api/data', (req, res) => {
  ...
  data.analytics.lastAccess = new Date().toISOString();
  saveData(data);
  res.json(data);
});
...
app.put('/api/quests/:id', (req, res) => {
  ...
  if (index !== -1) {
    ...
    saveData(data);
    res.json({ success: true, quest: data.quests[index] });
  } else {
...
app.get('/api/admin', (req, res) => {
  const data = loadData();
  const adminData = {
    totalQuests: data.quests.length,
    totalMarkers: data.markers.length,
    questsOpen: data.quests.filter(q => q.status === 'open').length,
    questsTaken: data.quests.filter(q => q.status === 'taken').length,
    activeUsers: new Set(data.quests.map(q => q.user)).size,
    markersToday: data.markers.filter(m => {
...
app.get('/api/export', (req, res) => {
  ...
  res.setHeader('Content-disposition', 'attachment; filename=simplebisnis_data.json');
...
app.post('/api/import', (req, res) => {
  try {
    ...
    const data = {
      ...
      analytics: {
        totalVisits: importData.analytics?.totalVisits || 0,
        lastAccess: new Date().toISOString()
      }
    };
...
// Serve HTML untuk semua route (SPA)
app.get('*', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="id">
                    ...
                    <div class="maps-controls" id="mapPanel">
                        ...
                        <div class="marker-list-container">
                            ...
                            <div class="marker-list" id="markerList">
                                <!-- Marker list akan diisi dari JavaScript -->
                            </div>
                        </div>
                    </div>
                    <div id="map"></div>
            ...
            <div id="quests" class="app-section">
                <div class="quest-section">
                    ...
                    <div class="quest-list" id="questListContainer">
                        <!-- Quest list akan diisi dari JavaScript -->
                    </div>
                ...
                <div class="admin-controls">
                    <div class="admin-card">
                        ...
                        <div class="admin-list" id="adminMarkerList">
                            <!-- Marker list akan diisi dari API -->
                        </div>
                    ...
                    <div class="admin-card">
                        ...
                        <div class="admin-list" id="adminQuestList">
                            <!-- Quest list akan diisi dari API -->
                        </div>
                    ...
                    <div class="admin-card">
                        ...
                        <div class="admin-list" id="adminCategoryList" style="margin-top: 1rem;">
                            <!-- Category list akan diisi dari API -->
                        </div>
                    </div>
                </div>
        ...
        <script>
            ...
            async function updateAdminStats() {
                try {
                    ...
                    document.getElementById('totalQuests').textContent = data.totalQuests;
                    document.getElementById('totalMarkers').textContent = data.totalMarkers;
                    document.getElementById('questsOpen').textContent = data.questsOpen;
                    document.getElementById('questsTaken').textContent = data.questsTaken;
                    document.getElementById('activeUsers').textContent = data.activeUsers;
                    document.getElementById('markersToday').textContent = data.markersToday;
                } catch (error) {
            ...
            function loadAdminMarkerList() {
                ...
                if (markers.length === 0) {
                    container.innerHTML = '<div style="text-align: center; padding: 1rem; color: var(--text-secondary);">Tidak ada markers.</div>';
                    return;
                }
                    ...
                    item.innerHTML = \`
                        <div>
                            ...
                            <div style="font-size: 0.8rem; color: var(--text-secondary); margin-top: 0.3rem;">
                                \${marker.desc.substring(0, 50)}\${marker.desc.length > 50 ? '...' : ''}
                            </div>
                            <span class="category-badge">\${marker.category}</span>
                        </div>
                        <div class="item-actions">
                            <button class="cyber-button btn-small btn-warning" onclick="editMarker(\${marker.id})">
            ...
            function loadAdminQuestList() {
                ...
                if (quests.length === 0) {
                    container.innerHTML = '<div style="text-align: center; padding: 1rem; color: var(--text-secondary);">Tidak ada quests.</div>';
                    return;
                }
                ...
                quests.forEach((quest, index) => {
                    ...
                    item.innerHTML = \`
                        ...
                        <div class="item-actions">
                            <button class="cyber-button btn-small btn-warning" onclick="editQuest(\${quest.id})">
                                <i class="fas fa-edit"></i>
                            </button>
            ...
            function loadAdminCategoryList() {
                ...
                const allCategories = [...new Set([
                    ...customCategories,
                    ...markers.map(m => m.category),
                    ...quests.map(q => q.category)
                ])];
                
                if (allCategories.length === 0) {
                    container.innerHTML = '<div style="text-align: center; padding: 1rem; color: var(--text-secondary);">Tidak ada kategori.</div>';
                    return;
                }
                ...
                allCategories.forEach((category, index) => {
                    ...
                    item.innerHTML = \`
                        <div>
                            ...
                            <div style="font-size: 0.8rem; color: var(--text-secondary); margin-top: 0.3rem;">
                                Used: \${markers.filter(m => m.category === category).length + quests.filter(q => q.category === category).length} times
                            </div>
                        </div>
                        <div class="item-actions">
                            <button class="cyber-button btn-small btn-danger" onclick="deleteCategory('\${category}')" \${customCategories.includes(category) ? '' : 'disabled'}>
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    \`;
            ...
            async function editMarker(id) {
                ...
                const newTitle = prompt('Edit judul marker:', marker.title);
                if (newTitle !== null) {
                    const newDesc = prompt('Edit deskripsi marker:', marker.desc);
                    if (newDesc !== null) {
                        try {
                            const response = await fetch(\`/api/markers/\${id}\`, {
                                method: 'PUT',
            ...
            async function editQuest(id) {
                ...
                const newTitle = prompt('Edit judul quest:', quest.title);
                if (newTitle !== null) {
                    const newDesc = prompt('Edit deskripsi quest:', quest.desc);
                    if (newDesc !== null) {
                        try {
                            const response = await fetch(\`/api/quests/\${id}\`, {
                                method: 'PUT',
            ...
            async function addNewCategory() {
                ...
                loadAdminCategoryList();
                alert(\`Kategori "\${newCategory}" berhasil ditambahkan!\`);
            }

            function deleteCategory(category) {
                if (confirm(\`Apakah Anda yakin ingin menghapus kategori "\${category}"?\`)) {
                    customCategories = customCategories.filter(cat => cat !== category);
                    loadAdminCategoryList();
            ...
            async function clearAllMarkers() {
                if (confirm('PERINGATAN! Anda yakin ingin menghapus SEMUA markers? Aksi ini TIDAK dapat dibatalkan!')) {
                    ...
                    loadAdminMarkerList();
                    await updateAdminStats();
                    alert('Semua markers berhasil dihapus!');
                }
            }

            async function clearAllQuests() {
                if (confirm('PERINGATAN! Anda yakin ingin menghapus SEMUA quests? Aksi ini TIDAK dapat dibatalkan!')) {
                    ...
                    loadAdminQuestList();
                    await updateAdminStats();
                    alert('Semua quests berhasil dihapus!');
                }
            }
            ...
            function renderMarkerList() {
                ...
                filteredMarkers.forEach((markerData) => {
                    ...
                    item.onclick = () => {
                        ...
                        // Buka popup jika ada marker di peta
                        const markerOnMap = markerLayerGroup.getLayers().find(layer => 
                            layer.getLatLng().lat === markerData.lat && 
                            layer.getLatLng().lng === markerData.lng
                        );
                        if (markerOnMap) markerOnMap.openPopup();
                    };
            ...
            async function takeOrder() {
                ...
                if(q) { 
                    try {
                        const response = await fetch(\`/api/quests/\${q.id}\`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                ...
                let html = \`
                    ...
                    <div style="margin-top:20px; padding: 10px; border: 1px dashed rgba(255,255,255,0.2); border-radius: 5px;">
                        <span style="color:var(--text-secondary); font-size:0.9rem; display: block; text-align: center; margin-bottom: 5px;">
                            ⚠️ **Info Lengkap (Registrar/Expired)** memerlukan Cek Manual:
                        </span>
            ...
            // --- SAMPLE DATA (jika API offline) ---
            function addSampleMarkers() {
                const sampleMarkers = [
                    { id: Date.now(), lat: -6.2000, lng: 106.8166, title: 'Macet di Bundaran HI', desc: 'Kemacetan parah akibat kecelakaan', category: 'traffic', createdAt: new Date().toISOString() },
                    { id: Date.now() + 1, lat: -6.1900, lng: 106.8200, title: 'Toko Elektronik', desc: 'Jual beli sparepart murah', category: 'shop', createdAt: new Date().toISOString() }
                ];
